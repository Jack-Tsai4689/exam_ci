...
<div id="all">
	...
	<div class="title_intro">
		<div class="top_search"><label style="margin-left:5px;">關鍵字搜尋</label><input type="text" class="input_field" name="q" id="q" value="<?=$Keyword?>"><div class="glass_div" onclick="search_confirm()"><img src="<?=base_url('images/icon_op_glass.png')?>"></div><a href="<?=site_url('/question')?>" style="margin-left:55px;">瀏覽全部</a></div>
	...
	<div class="title_intro condition">
		<div>
			...
			　　<input type="button" class="btn" id="joinsets" value="勾選加入考卷">
		</div>
	</div>
	</form>
	<div class="content">
			...
			<table cellpadding="0" cellspacing="0" width="100%" class="list">
				<thead>
					<tr>
						...
						<th class="last" style="width:120px;">編輯</th>
					</tr>
				</thead>
				<tbody>
				...
				?>
					<tr class="<?=$class?>">
						...
						<td class="qcont" name="que"><?=$v->QCONT.'<br>'.$v->ACONT.$v->KCONT?></td>
						<td><?=$v->QUE_TYPE?></td>
						...
						<td><?=$v->UPDATETIME?></td>
						<td class="last"><input type="button" class="btn w50" onclick="viewq(<?=$v->QID?>)" value="明細">　<? if ($v->QUE_TYPE_EN!=="G"):?><input type="button" class="btn w50" onclick="editq(<?=$v->QID?>)" value="編輯"><?php endif; ?></td>
					</tr>
				<?php endforeach; ?>
				</tbody>
			</table>
...
function viewq(q){
  	window.open("<?=site_url('/question/show').'/'?>"+q,"_blank","width=800,height=600,resizable=yes,scrollbars=yes,location=no");
}
function editq(q){
...
function ques_find(){
	location.href = '<?=site_url('/question')?>?'+$('#form1').serialize();
}
